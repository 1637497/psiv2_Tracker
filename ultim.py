...
output_filename = 'bo_output7_80.mp4'
...
while True:
    ...
    for (objectID, centroid) in objects.items():
        for (startX, startY, endX, endY) in prev_bboxes:
            adj_startX, adj_startY = startX , startY + 500
            adj_endX, adj_endY = endX , endY + 500
            cv2.rectangle(frame, (adj_startX, adj_startY), (adj_endX, adj_endY), (0, 255, 0), 2)
            cv2.putText(frame, f"ID {objectID}", (centroid[0] - 10, centroid[1] + 500 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
            cv2.circle(frame, (centroid[0] , centroid[1] + 500), 4, (0, 255, 0), -1)
