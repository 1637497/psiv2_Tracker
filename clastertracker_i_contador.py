from scipy.spatial import distance as dist
from collections import OrderedDict
import numpy as np
import cv2
import torch
from time import time

# Load the video and YOLOv5 model
cap = cv2.VideoCapture('output7.mp4')  # Make sure to use the correct video path
model1 = torch.hub.load('ultralytics/yolov5', 'custom', path='/Users/guillemsallas/Desktop/Universitat/4t curs/1r Semestre/PSIV/PSIV2_Detecció_cotxes/best50.pt')

fps = cap.get(cv2.CAP_PROP_FPS)
frame_width = int(cap.get(3))
frame_height = int(cap.get(4))

out = cv2.VideoWriter('Users/guillemsallas/Desktop/Universitat/4t curs/1r Semestre/PSIV/PSIV2_Detecció_cotxes/resultat.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (frame_width, frame_height))

# Function for detecting vehicles with YOLOv5
def detectar(i):
    ll = []
    
    results = model1(i)  # YOLO detection

    ims = np.squeeze(results.render())  # Render results on the frame
    
    df = results.pandas().xyxy[0]  # Get the bounding boxes as pandas dataframe
    df = df[df['confidence'] > 0.75]  # Filter boxes by confidence score
    
    df = df.to_dict(orient='records')
    
    if df:
        for vehicle in df:
            xmin = round(vehicle['xmin'])
            ymin = round(vehicle['ymin'])
            xmax = round(vehicle['xmax'])
            ymax = round(vehicle['ymax'])
            ll.append((xmin, ymin, xmax, ymax))
    
    return ll

# CentroidTracker class for tracking objects
class CentroidTracker:
    def __init__(self, maxDisappeared=25,maxDistance=50):
        self.nextObjectID = 0
        self.objects = OrderedDict()
        self.disappeared = OrderedDict()
        self.maxDisappeared = maxDisappeared
        self.maxDistance=maxDistance

    def register(self, centroid):
        self.objects[self.nextObjectID] = centroid
        self.disappeared[self.nextObjectID] = 0
        self.nextObjectID += 1

    def deregister(self, objectID):
        del self.objects[objectID]
        del self.disappeared[objectID]
        
    def trobar_minims(self,diccionari):
        resultats = []
        for k, v in diccionari.items():
            # Trobar la posició del valor mínim
            pos_minim = np.argmin(v)
            # Afegir la parella (k, posició del mínim) a la llista de resultats
            resultats.append((k, pos_minim))
        return resultats

    def update(self, rects):
        if len(rects) == 0:
            for objectID in list(self.disappeared.keys()):
                self.disappeared[objectID] += 1
                if self.disappeared[objectID] > self.maxDisappeared:
                    self.deregister(objectID)
            return self.objects

        inputCentroids = np.zeros((len(rects), 2), dtype="int")
        for (i, (startX, startY, endX, endY)) in enumerate(rects):
            cX = int((startX + endX) / 2.0)
            cY = int((startY + endY) / 2.0)
            inputCentroids[i] = (cX, cY)

        if len(self.objects) == 0:
            for i in range(len(inputCentroids)):
                self.register(inputCentroids[i])
        else:
            objectIDs = list(self.objects.keys())
            objectCentroids = list(self.objects.values())
            
            
            D = dist.cdist(np.array(objectCentroids), inputCentroids)
            print("Distàncies:",D)
            
            
            #Creem diccionari on la clau és l'ID dels centroids que ja 
            #teníem i els valors son la distància a cada un dels centroids nous
            diccionari = {i: D[i] for i in range(D.shape[0])}
            diccionari = dict(sorted(diccionari.items(), key=lambda x: min(x[1])))
            #Apliquem una funció que troba la posició del mínim
            #dels valors per a cada clau
            #Osigui retorna per cada clau la següent parella:
            #(Clau, posició de la distància mínima)
            resultat = self.trobar_minims(diccionari)

            #Creem 2 llistes amb els IDS
            idsactuals=list(range(D.shape[0]))
            idsnous=list(range(D.shape[1]))

            #Iterem sobre les parelles
            for parella in resultat:
                actual=parella[0]
                nou=parella[1]
                
                #Comprovem si la distància és major que maxDistance
                if D[actual,nou]>self.maxDistance:
                    continue
                
                #Mirem si ja hem revisat el centroid
                if actual not in idsactuals or nou not in idsnous:
                    continue
                
                #Aconseguim l'ID del centroid actual
                objectID = objectIDs[actual]
                #Ara actualitzem el centroid actual amb el valor del nou
                self.objects[objectID] = inputCentroids[nou]
                #Posem desaparegut a 0
                self.disappeared[objectID] = 0
                #Borrem de les llistes per indicar que ja els hem revisat
                idsactuals.remove(actual)
                idsnous.remove(nou)
                
            #Ara a les llistes nomes queden aquells centroids que no hem revisat
            #Per tant, si hi ha més idsnous que actuals, haurem de registrar-los
            if len(idsnous)>len(idsactuals):
                for id in idsnous:
                    self.register(inputCentroids[id])
            
            #Però si n'hi ha més d'actuals que de nous,
            #significarà que alguns hauran desaparegut
            else:
                for id in idsactuals:
                    objectID = objectIDs[id]
                    self.disappeared[objectID] += 1
                    if self.disappeared[objectID] > self.maxDisappeared:
                        #Si porta massa temps desaparegut, l'eliminem
                        self.deregister(objectID)

        return self.objects


tracker = CentroidTracker()
t0 = time()

i = 0
prev_bboxes = []
car_count = 0
crossed_line_ids = set()  # To track cars that have already crossed the line

# Define the position of the detection line
line_y_position = 900  # Adjust this based on your video

while True:
    i += 1
    ret, frame = cap.read()

    if not ret:
        break

    roi = frame[340:960, 50:380]  # Region of Interest
    if i % 4 == 0:  # Run YOLO detection every 4th frame
        bboxes = detectar(roi)  # Detect vehicles
        if bboxes:
            prev_bboxes = bboxes
        objects = tracker.update(bboxes)
    else:
        objects = tracker.update(prev_bboxes)

    # Draw detection line on the frame
    cv2.line(frame, (0, line_y_position), (frame_width, line_y_position), (0, 0, 255), 2)

    # Display car tracking and counting
    for (objectID, centroid) in objects.items():
        for (startX, startY, endX, endY) in prev_bboxes:
            adj_startX, adj_startY = startX + 50, startY + 340
            adj_endX, adj_endY = endX + 50, endY + 340

            # Draw rectangles around cars
            cv2.rectangle(frame, (adj_startX, adj_startY), (adj_endX, adj_endY), (0, 255, 0), 2)

            # Check if a car crossed the detection line
            if centroid[1] + 340 >= line_y_position and objectID not in crossed_line_ids:
                car_count += 1
                crossed_line_ids.add(objectID)

        # Label the cars with their IDs
        cv2.putText(frame, f"ID {objectID}", (centroid[0] + 50 - 10, centroid[1] + 340 - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
        cv2.circle(frame, (centroid[0] + 50, centroid[1] + 340), 4, (0, 255, 0), -1)

    # Display the total car count on the frame
    cv2.putText(frame, f"Total Cars: {car_count}", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 255, 255), 3)

    cv2.imshow('Tracking', frame)
    out.write(frame)

    key = cv2.waitKey(30)
    if key == 27:
        break

cap.release()
out.release()
cv2.destroyAllWindows()


# Display total frames and time taken
t = time()
print("Número total de frames:", i)
print("Ha trigat", t - t0, "segons")
print("Ha trigat", (t - t0) / 60, "minuts")
